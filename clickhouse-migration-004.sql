...
--    003의 증거: otel_metrics_sum.SeriesKey의 default_expression이 StartTimeUnix를 포함하는지.
--      StartTimeUnix 자체는 원래 있는 기본 컬럼이라 존재만으로는 아무것도 증명하지 않는다.
--      dashboard/server/schema.js의 프로브는 메타데이터가 아니라 최근 행의 키(데이터)를 보므로
--      여기서 재사용할 수 없다.
