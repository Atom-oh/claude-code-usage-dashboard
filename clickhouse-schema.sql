...
-- 006: dedicated Codex metric tables (additive migration 006; requires ledger 004).

CREATE TABLE IF NOT EXISTS claude_code.codex_metrics_sum
(
    ResourceAttributes Map(LowCardinality(String), String) CODEC(ZSTD(1)),
...
TTL toDateTime(TimeUnix) + INTERVAL 180 DAY DELETE
SETTINGS ttl_only_drop_parts = 1;

CREATE TABLE IF NOT EXISTS claude_code.codex_metrics_gauge
(
    ResourceAttributes Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    ResourceSchemaUrl String CODEC(ZSTD(1)),
    ScopeName String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS claude_code.codex_metrics_histogram
(
    ResourceAttributes Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    ResourceSchemaUrl String CODEC(ZSTD(1)),
    ScopeName String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS claude_code.codex_metrics_exponential_histogram
(
    ResourceAttributes Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    ResourceSchemaUrl String CODEC(ZSTD(1)),
    ScopeName String CODEC(ZSTD(1)),
...
INSERT INTO claude_code.schema_migrations (version, name, checksum) SELECT 6, '006-codex-native-signals', '78098beb9713662bdbe066d3ae5e96d1cdf724282e097ec19e68b9229704ccbd'
FROM system.one
