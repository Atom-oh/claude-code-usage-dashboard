...
# 멈춤 방지. 셀이 비고 종료코드가 0이 아니면 최대 PANEL_RETRIES 회 재시도(gpt-5.6-sol/
...
LENSES_DIR="$2"; WORK="$3"
# $WORK 가 비면 ensure_slots 의 `rm -rf "$1/slot"` 가 `rm -rf /slot`(파일시스템 루트
# 하위) 이 되는 파괴적 경로가 생긴다. $LENSES_DIR 빈 값은 파괴적이진 않지만(글롭이 매치
# 없이 조용히 0셀로 끝남) 인자 오설정을 조용히 넘기지 않고 바로 잡아내는 게 디버깅에 낫다.
...
# kiro-cli 2.11.1 은 `chat --help` 에 여전히 "trust no tools: '--trust-tools='" 를 적어
# 두지만, 실제로는 빈 값을 커스텀 툴 이름 "" 로 해석해 `WARNING: --trust-tools arg for
# custom tool  needs to be prepended with @{MCPSERVERNAME}/` 만 찍고 **무시**한다 —
# 내장 툴 이름이 fs_read/fs_write → read/write/shell/glob/grep/code/aws… 로 바뀌면서 기본
# 에이전트의 "trust working directory"(read/glob/grep/code)·"trust read-only"(aws) 신뢰가
# 그대로 살아남는다. 로컬 재현(2.11.1, headless): `--trust-tools=` 로도 cwd 안 파일을
# `read` 로 읽어 내용을 그대로 출력했다(cwd 밖 절대경로만 non-interactive 거부). 반면
...
for lens_file in "${LENS_FILES[@]}"; do
  ...
  # ~/.codex/config.toml 이 결정하며, 그 값이 global.openai.gpt-6-astra; KIRO_MODELS 의
  ...
  # amazon-bedrock-runtime 을 통한 global 모델이라 region pinning 이 더는 필요 없다
...
# (예: kiro-cli 플래그/`--agent` 가 이 러너에서 무효거나 모델 ID 가
...
for model_tag in codex "${KIRO_MODELS[@]##*:}"; do
  ...
  # 생길 수 있다. $RESP 는 run-panel.sh 시작부에 항상 만들어지므로 "파일 없음" 폴백
