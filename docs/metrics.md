...
아래 다섯 가지는 여러 지표에 공통으로 적용되는 주의사항이라 각 항목에서 반복하지 않고 번호로
참조한다.
...
2. **비용은 하한선 (cost is a lower bound)** — 계산 비용(computed cost)은 계측되지 않은 실행
   경로, 단가표에 없는 모델(아래 3번), 텔레메트리가 없는 채널을 반영하지 못한다. Cost/Executive
   페이지의 안내 문구가 이를 알린다.
...
- **정의**: 선택한 기간에 세션이 1건 이상 있었던 고유 유저 수(그룹 무관, `unknown` 세션 포함).
...
### 개발자당 지출 (Spend per Developer)
- **정의**: 기간 지출(계산) ÷ 활성 개발자 수.
...
- **정의**: CLI 구동 시간 ÷ 사용자 상호작용 시간 — 값이 클수록 에이전트가 사람 개입 없이
  더 오래 자율 실행했다는 뜻.
...
### 전체 유저 (Total Users)
- **정의**: 선택 기간에 세션이 1건 이상 있었던 고유 유저 수(그룹 무관).
...
### 추가 라인 (Lines Added)
- Executive의 "작성 라인"과 동일 — `queries.js:kpiSummary`.
...
### 전체 멤버 (Total Members)
- **정의**: 조직 전체 이력에서 한 번이라도 세션을 실행한 고유 유저 수(선택 기간과 무관, lookback
  전체).
- **원천**: `claude_code.session.count`.
...
### 월간 활성 (MAU)
- **정의**: 조회 종료 시점 기준 최근 30일 내 세션이 있었던 고유 유저 수.
- **원천**: `claude_code.session.count`.
...
### 주간 활성 (WAU)
- **정의**: 조회 종료 시점 기준 최근 7일 내 세션이 있었던 고유 유저 수.
- **계산**: `queries.js:adoptionLevels`(스냅샷), 시계열은 `activity.js:rollupAdoption`
  (당일 포함 trailing 7일 union).
- **주의**: 4번.

### 일간 활성 (DAU)
- **정의**: 조회 종료 시점(또는 시계열의 해당 날짜) 세션이 있었던 고유 유저 수.
- **계산**: `queries.js:adoptionLevels`(스냅샷), 시계열은 `activity.js:rollupAdoption`.
- **주의**: 4번.

### DAU/MAU 고착도 (Stickiness)
- **정의**: 일간 활성 유저 수 ÷ 월간 활성 유저 수 — 값이 높을수록 월간 사용자 중 많은 비율이
...
### 계산 비용 (합계) (Computed Cost, Total)
- Executive의 "기간 지출 (계산)"과 동일한 원천 — `queries.js:costSummary`, `pricing.js:withComputedCost`.
- **주의**: 2번, 3번, 4번.

### 보고 비용 (Claude Code) (Reported Cost)
...
### 30일 프로젝션 (30-Day Projection)
- Executive의 "30일 프로젝션"과 동일한 정의(일평균 × 30), Cost 페이지 자체 합계 기준으로 재계산.
- **주의**: 2번, 3번.

### 개발자당 지출 (Spend per Developer)
- Executive의 "개발자당 지출"과 동일 — 계산 비용 총계 ÷ `activeUsers`(전체 개발자 수).
- **계산**: `Cost.jsx`의 `spendPerDeveloper`.
- **주의**: 2번, 3번, 4번.

### 사용자당 평균 — 그룹별 (Spend per User, by Group)
- **정의**: 그룹별 계산 비용 ÷ 그 그룹의 고유 유저 수 — 그룹 간 인원수 차이를 상쇄한 비교용 지표.
- **계산**: `Cost.jsx`의 `spendPerUserFor`.
- **주의**: 2번, 3번, 5번(그룹별 인원 합은 전역 uniq보다 클 수 있음).

### 캐시 티어별 지출 / 캐시율 (Spend by Cache Tier / Cache Reuse Ratio)
- Overview의 "캐시 효율"과 같은 `cache_read_ratio`를 재사용 — `queries.js:cacheEfficiency`.
- **주의**: 캐시 쓰기를 분모에서 빼면 안 된다(위 Overview 항목과 동일한 주의).

### Effort별 지출 (Spend by Effort)
- **정의**: `effort`(사고 강도) 속성별 비용 비중 — `effort` 속성이 없는 행은 `unknown`으로 묶는다.
- **원천**: `claude_code.cost.usage`.
...
### 생성된 PR / 작성된 라인 수 (PRs Created / Lines Written)
...
### 유저별 생산성 리더보드 (Per-User Productivity Leaderboard)
- 각 컬럼(생산성 점수, 세션, 토큰, 추가 라인, PR, 커밋, 수락률, 활성일)은 위 Executive/
...
### 그룹별 요약 — 인원/평균 생산성 점수/세션/수락률 (Group Summary)
- **정의**: 리더보드 행을 그룹별로 묶어 평균낸 요약(수락률은 단순 평균이 아니라
  수락/결정 건수 가중 평균).
- **계산**: `Users.jsx`의 `GroupFaceOff`.
- **주의**: 5번.

### 모델 계열별 사용자당 평균 지출 (Spend per User, by Model Family)
- **정의**: 모델 계열(예: Claude 3.x, 4.x)을 실제로 쓴 유저 수로 나눈 평균 지출.
- **계산**: `계산 규칙은 \`Users.jsx:familyStats\` 참고` — 분자·분모 모두 단가표에 있는(priced)
  모델 사용 행만으로 계산한다.
- **주의**: 3번. 한 유저가 여러 계열을 쓰면 계열마다 분모에 중복으로 들어간다.

### 유저별 도구/Skill 사용 내역 (Per-User Tool / Skill Usage)
- **정의**: 유저별 도구 호출 횟수 / Skill 호출 횟수.
...
이 페이지는 StatTile이 아니라 표(DataTable)로만 지표를 보여준다. 표에 나오는 값(API 레이턴시
p50/p95, refusal 건수, 재시도 소진 건수, 버전 코호트별 세션 수/비용)의 정의는 각 표의 부제
(subtitle)에 이미 있고, 원천/계산은 다음과 같다: `queries.js:apiLatency`, `queries.js:refusalRate`,
`queries.js:retriesExhausted`, `queries.js:apiErrors`, `queries.js:versionCohortSessions`,
`queries.js:versionCohortCost`. 이 페이지의 목적 자체가 "A/B 비교를 신뢰해도 되는가"를 확인하는
것이라, 개별 수치보다 그룹 간 비대칭 여부가 핵심이다(표 부제 참고).

## Usage

이 페이지도 StatTile 없이 표로만 지표를 보여준다(도구/MCP 성공률, 커맨드 사용, Skill 근사 비용,
플러그인 인벤토리 등). 원천/계산은 `queries.js:toolMcpUsage`, `queries.js:mcpConnectorUsage`,
`queries.js:commandAdoption`, `queries.js:skillUsage`, `queries.js:pluginInventory`,
`queries.js:subagentFanout`, `queries.js:compactionPressure`다. Skill의 "근사 비용"은
`claude_code.cost.usage`의 `SkillName` 속성을 이용한 근사치이고(주의 3번 대상 모델은 여기서도
제외), 나머지는 카운트/비율 지표라 캐치올 주의사항이 따로 붙지 않는다.
