...
| `GET /api/cost/by-model` | Cost/tokens per group x model. *(2026-09-09)* Every priced row here and in the other `withComputedCost` consumers also carries `cache_write_ttl` (`"5m"`, `"1h"`, `"mixed"` when a range spanned a policy switch, `null` when unpriced) and `cache_write_cost` — the tier actually applied to that row's cache-creation tokens and that term's dollar amount (ADR-008) |
| `GET /api/cost/by-user-model` | Cost/tokens per user x group x model (real session group, not majority-vote) |
| `GET /api/cost/by-model-daily` | *timeseries* — cost per group x model over time. Each bucket is priced with the TTL policy in force at its start (`day`); a switch instant inside a bucket is not split, so put switch instants on UTC midnight for day buckets |
| `GET /api/cost/by-model-compare` | Current vs. previous equal-length period, per model. *(2026-09-09)* Queried at a group x model grain (the cache-write tier is per group) and folded back to `model`; when a TTL switch instant falls inside either window it falls back to two `by-model`-style calls, whose hour alignment can differ from the single-query path by up to 59 minutes |
| `GET /api/cost/tiers` | Cost broken down by token tier (uncachedInput/cacheRead/cacheWrite/output), split by group: `{"bedrock": {...}, "enterprise": {...}}`. `cacheWrite` is summed from the priced rows' `cache_write_cost`, not re-derived from tokens |
| `GET /api/cost/effort-mix` | *(2026-09-01, computed cost since 2026-09-04)* (`effortMix`) Per group x effort level: `cost` (computed — tokens × `pricing.js` rates, the same basis as every other Cost card) + `reported_cost` (Claude Code's `cost.usage`, kept for contrast) + `tokens` + `unpriced_tokens`. Reported cost is priced client-side and therefore moves with the client version (measured 2026-09-03: v2.1.251 prices `claude-fable-5-1` off the opus-5 row → ≈0.5× of list, v2.1.258 at list), which is why it is no longer the panel's basis. Effort isn't an `incFlat` dimension, so this is a self-contained session-boundary local diff (ADR-001 pattern) — now at a `model` grain, folded to group x effort in JS by `pricing.js`'s `rollupComputedCost()`. `effort = ''` (rows with no effort attribute; measured 7d cost 578 vs medium 4,743 / high 1,576 / xhigh 307) → `'unknown'`; the `Speed` column is ignored (measured 0 rows fleet-wide). Cost page. |
| `GET /api/cost/by-agent` | *(2026-09-01, computed cost since 2026-09-04)* (`agentCost`) Per group x subagent (`AgentName`, measured 7d: 4.56M non-empty rows; `'' → 'main'` = main-thread work): `cost` + `reported_cost` + `tokens` + `unpriced_tokens` on the same basis and the same `rollupComputedCost()` fold as `effort-mix`. Ordered by computed cost, top 30 (applied in JS after the fold, not as a SQL `LIMIT`). Cost page. |
...
- **`pricing`** — `{"cacheWriteTtl": "1h"|"5m", "cacheWriteTtlByGroup": {"bedrock": [{"since": ISO|null, "ttl": "1h"|"5m"}], "enterprise": [...]}, "overriddenModels": [str]}`. `pricing.cacheWriteTtl` is the global cache-write tier from `PRICING_CACHE_WRITE_TTL` ("1h" default), applied to the `unknown` group and to rows without a group; `pricing.cacheWriteTtlByGroup` *(2026-09-09, ADR-008)* is the per-group policy from `PRICING_CACHE_WRITE_TTL_BEDROCK` / `_ENTERPRISE` (defaults bedrock `5m` / enterprise `1h`, matching Claude Code's `promptCacheTtl` defaults per auth channel; a schedule lists the instants the tier switched at). Surfaced because OTel's cache-creation TokenType can't distinguish 5m from 1h writes, so the tier behind a cost figure is a policy, not a measurement — each priced row also states the tier it used in `cache_write_ttl`. `pricing.overriddenModels` lists the model keys whose rates came from `PRICING_JSON` (empty on a default deploy), so a viewer can tell built-in list prices from an operator's negotiated ones. As of 2026-09-02 the built-in table also covers `claude-fable-5-1` / `claude-mythos-5` / `claude-mythos-5-1` / `claude-opus-4-1` / `claude-opus-4` / `claude-sonnet-4` (the `-5-1` pair carries an explicit `cacheRead` of `$0.25`, a 0.025x exception to the usual 0.1x derivation), and the geo-prefix normalization now strips `us-gov.` / `jp.` / `au.` in addition to `us.` / `eu.` / `apac.` / `global.`. The rates themselves are deliberately not exposed by this endpoint. `pricing` is informational — the SPA does not currently read it.
- **`schema`** — `{"segmentAwareSeriesKey": bool|null, "migrations": int[]|null}`. `true`/`false`/`null`: `true` means the cluster has migration-003's segment-aware `SeriesKey` expression in force, `false` means it still has the legacy expression, and `null` means undetermined — it is probed from the newest `claude_code.cost.usage` rows (at boot and every 10 minutes) rather than assumed, and mixed keys mid-`MATERIALIZE COLUMN`, no recent rows, or any probe error all collapse to `null`. No SPA copy consumes this field any more: the callout that used to read it was removed, so `segmentAwareSeriesKey` is exposed for operators and other `/api/config` consumers. `migrations` is the sorted, deduplicated list of migration versions read from `claude_code.schema_migrations` (`clickhouse-migration-004.sql`), probed on the same boot-plus-10-minute timer as `segmentAwareSeriesKey`. `null` means undetermined — including a cluster that predates `clickhouse-migration-004.sql`, where the ledger table does not exist yet — and `[]` means the ledger exists but is empty. `schema` is informational — the SPA does not read it.
