...
- **`schema`** — `{"segmentAwareSeriesKey": bool|null, "migrations": int[]|null}`. `true`/`false`/`null`: `true` means the cluster has migration-003's segment-aware `SeriesKey` expression in force, `false` means it still has the legacy expression, and `null` means undetermined — it is probed from the newest `claude_code.cost.usage` rows (at boot and every 10 minutes) rather than assumed, and mixed keys mid-`MATERIALIZE COLUMN`, no recent rows, or any probe error all collapse to `null`. The SPA's `LowerBoundNote` reads it fail-safe: only `true` drops the `--resume` cause from the lower-bound callout, and every other value keeps the full warning. `migrations` is the sorted, deduplicated list of migration versions read from `claude_code.schema_migrations` (`clickhouse-migration-004.sql`), probed on the same boot-plus-10-minute timer as `segmentAwareSeriesKey`. `null` means undetermined — including a cluster that predates `clickhouse-migration-004.sql`, where the ledger table does not exist yet — and `[]` means the ledger exists but is empty. `schema` is consumed by the SPA.
