...
- **ClickHouse (`otel_metrics_sum_hourly`)** -- `ReplicatedAggregatingMergeTree` hourly rollup
  ...
  Cumulative counters keep `max(Value)` per (SeriesKey, SessionId, hour). It carries the same
  90-day cold / 180-day delete TTL as the raw table: it holds `UserEmail`, so an untilled rollup
...
- **ClickHouse(`otel_metrics_sum_hourly`)** -- `otel_metrics_sum` 위의 materialized view가
  ...
  누적 카운터는 (SeriesKey, SessionId, hour)당 `max(Value)`만 보존한다. TTL은 원본과 동일하게
  90일 cold 이동 / 180일 삭제다 — `UserEmail`을 담는 저장소라 TTL이 없으면 원본이 삭제된 뒤에도
