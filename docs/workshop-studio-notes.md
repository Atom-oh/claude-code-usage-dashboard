...
- **Ask Claude 챗(Bedrock) 배포 전제**: 대시보드의 Ask Claude 기능은 Bedrock을 호출하므로 아래가 갖춰져야 뜬다(안 그러면 대시보드는 떠도 챗만 `AccessDenied`/timeout이 난다).
  - **Bedrock model access 활성화**: admin 계정에서 `CHAT_MODEL_ID`(기본 `global.anthropic.claude-sonnet-5` — `global.` 프리픽스가 붙은 global inference profile) 모델의 access를 켠다. 파드에는 `BEDROCK_REGION = var.bedrock_region`이 주입되고 `chat.js`가 `BEDROCK_REGION → AWS_REGION → us-east-1` 순으로 골라 Bedrock 클라이언트 리전을 정한다 — **`var.bedrock_region` 기준으로 이 global 프로파일이 가용해야 한다.** 지금 이 admin 환경(현재 계정)은 워크샵 계정이 아니라 리전 제한이 없으므로 기본값을 `var.region`과 같은 `ap-northeast-2`로 둔다. **이미지를 빌드해 워크샵 계정으로 이식할 때는 `var.bedrock_region`을 `us-west-2`(워크샵 계정이 Bedrock 호출을 허용하는 유일한 리전)로 바꿔서 apply해야 한다** — 이미지 자체는 그대로 재사용 가능, env만 바뀐다. `var.bedrock_region = ""`이면 env 주입을 생략하고 `AWS_REGION`(= `var.region`)을 그대로 쓴다.
  ...
  - **env / 모델 변경**: 모델을 바꾸려면 `var.chat_model_id` 변수 하나만 고치면 env(`CHAT_MODEL_ID`)와 IAM policy ARN이 함께 반영된다(코드/IAM 두 곳 수동 동기화 불필요). Bedrock 호출 리전은 `var.bedrock_region`, 인프라 리전은 `var.region`.
