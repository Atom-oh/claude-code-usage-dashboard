...
- `dashboard/server/chat.js:233` -- `SYSTEM` prompt (intro + `SCHEMA_CONTEXT` + `GROUP_CTE` interpolation + output rules)
- `dashboard/server/chat.js:248` -- `TOOLS` (the `run_sql` tool spec)
- `dashboard/server/chat.js:278` -- `capToolResultJson()` (char-capped tool results, guards against cross-hop context growth)
- `dashboard/server/chat.js:290` -- `classifyChatError()` (AWS error -> user-facing Korean message + requestId)
- `dashboard/server/chat.js:320` -- `MAX_SQL_CALLS` (total run_sql executions per turn, independent of `MAX_HOPS` round-trips)
- `dashboard/server/chat.js:323` -- `handleChat()` (SSE stream + tool-use loop; wires an `AbortController` to `res`'s `close` event so a dropped browser connection cancels the in-flight Bedrock call and ClickHouse query, not just the SSE write)
...
- `dashboard/server/chat.js:233` -- `SYSTEM` 프롬프트(intro + `SCHEMA_CONTEXT` + `GROUP_CTE` 보간 + 출력 규칙)
- `dashboard/server/chat.js:248` -- `TOOLS`(`run_sql` 툴 스펙)
- `dashboard/server/chat.js:278` -- `capToolResultJson()`(문자수 상한 툴 결과 — hop 간 컨텍스트 팽창 방지)
- `dashboard/server/chat.js:290` -- `classifyChatError()`(AWS 에러 → 사용자용 한국어 문구 + requestId)
- `dashboard/server/chat.js:320` -- `MAX_SQL_CALLS`(한 턴의 총 run_sql 실행 상한 — 왕복 수인 `MAX_HOPS`와는 별개 축)
- `dashboard/server/chat.js:323` -- `handleChat()`(SSE 스트림 + 툴콜 루프; `res`의 `close` 이벤트에 `AbortController`를 연결해 브라우저 연결이 끊기면 SSE write뿐 아니라 진행 중인 Bedrock 호출·ClickHouse 쿼리도 취소)
