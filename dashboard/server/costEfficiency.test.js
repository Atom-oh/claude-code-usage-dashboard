...
test("userCostEfficiency joins per-user cost onto loc_added/commits and derives unit costs", () => {
  const leaderboard = [
    { user: "a@x.com", group: "bedrock", loc_added: 100, commits: 4 },
    { user: "b@x.com", group: "enterprise", loc_added: 0, commits: 0 },
  ];
...
test("userCostEfficiency flags unpriced users instead of ranking them as cost=0 most-efficient", () => {
  const leaderboard = [{ user: "c@x.com", group: "bedrock", loc_added: 500, commits: 10 }];
  const costByUserModel = [{ user: "c@x.com", model: "some-unknown-model", cost: null }];
  const out = userCostEfficiency(leaderboard, costByUserModel);
  assert.equal(out[0].unpriced, true);
  assert.equal(out[0].cost, 0);
  // loc_added=500 > 0이지만 unpriced라 cost_per_loc/commit은 null이어야 한다 —
