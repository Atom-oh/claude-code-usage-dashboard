...
test("delta exports deduplicate retries and remain separate from billing usage", () => {
  ...
  assert.equal(out.metrics[0].value, 5);
...
test("cumulative counters use baseline differences and distinguish process starts", () => {
  ...
  assert.equal(out.metrics[0].value, 10);
  assert.equal(out.metrics[0].partial, false);
});
test("missing cumulative baseline and invalid samples withhold the affected aggregate", () => {
  for (const row of [{ ...point, temporality: 2 }, { ...point, value: null }, { ...point, flags: 1 }]) {
    ...
    assert.equal(out.metrics[0].value, null);
...
test("histograms merge counts and sums, not means or cumulative observations", () => {
  ...
  assert.equal(out.metrics[0].count, 3);
  assert.equal(out.metrics[0].sum, 120);
  assert.equal(out.metrics[0].mean, 40);
  assert.equal(out.metrics[0].min, 10);
  assert.equal(out.metrics[0].max, 90);
  assert.equal(out.metrics[0].p95, undefined);
...
test("exporter-ambiguous zero histogram fields keep counts but withhold unknown statistics", () => {
  ...
  assert.equal(combined.mean, null);
  ...
  assert.equal(zeroIncrement.count, 1);
  assert.equal(zeroIncrement.mean, 0);
});
test("gauge observations are not added as usage, and metric labels retain scope", () => {
  ...
  assert.equal(gauge.value, 8);
  assert.equal(gauge.mean, 5);
...
test("preaggregated deltas carry export counts, last time and invalid samples", () => {
  ...
  assert.equal(result.metrics[0].value, 180002);
  assert.equal(result.metrics[0].points, 60001);
  ...
  assert.equal(invalid.metrics[0].value, null);
  assert.equal(invalid.metrics[0].partial, true);
...
test("preaggregated histogram means are observation weighted and missing extremes stay null", () => {
  ...
  assert.equal(result.metrics[0].points, 101);
  assert.equal(result.metrics[0].count, 201);
  assert.equal(result.metrics[0].mean, 5);
  assert.equal(result.metrics[0].min, null);
  assert.equal(result.metrics[0].max, null);
  assert.equal(result.metrics[0].p95_ms, undefined);
});
...
test("trace windows retain the parent relationship and never sum overlapping span time", () => {
  ...
  assert.equal(out.traces[0].wall_ms, 100);
  assert.equal(out.traces[0].span_count, 2);
  assert.equal(out.traces[0].errors, 1);
...
test("trace p95 uses empirical nearest rank, consistent with log latencies", () => {
  ...
  assert.equal(result.spans[0].average_ms, 150);
  assert.equal(result.spans[0].p95_ms, 200);
});
test("conflicting span identities fail explicitly regardless of input order", () => {
  for (const change of [{ duration_ns: 200_000_000 }, { name: "other" }, { parent_span_id: "parent" },
    ...
    for (const rows of [[span, conflict], [conflict, span]])
      assert.throws(() => foldCodexTraces(rows), /conflicting span/i);
  }
});
test("spans expose only bounded structured identifiers alongside timing", () => {
  ...
  assert.equal(output.attributes, undefined);
  assert.equal(output.input_tokens, undefined);
  assert.equal(output.cost_usd, undefined);
