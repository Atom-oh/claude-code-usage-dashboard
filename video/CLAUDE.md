...
`assets/`와 `capture/assets/`는 `site/assets/img/`의 복사본이라 커밋하지 않는다(같은 PNG가
저장소에 3벌 남는 것을 피함). 다시 렌더하기 전에 복원:

```bash
mkdir -p assets capture/assets
cp ../site/assets/img/*.png capture/assets/
cp ../site/assets/img/{overview,users,cost,analytics}.png assets/
```

렌더는 Node >= 22 + Chromium/ffmpeg 경로가 필요하다:

```bash
npx hyperframes check
npx hyperframes render --quality high --output renders/video.mp4
# 이 환경(Linux arm64)에서는 번들 브라우저/ffmpeg가 없어 경로를 직접 지정해야 했다:
