...
class CodexStorageTests(unittest.TestCase):
    ...
    def test_migration_and_all_exporter_insert_shapes(self):
        ...
        # Compare runnable blocks, not explanatory comments. Only storage topology
        # differs; exporter columns and guarded ledger evidence must stay identical.
        def localize(text):
            ...
            text = text.replace(" + INTERVAL 90 DAY TO VOLUME 'cold',\n    toDateTime(TimeUnix)", "")
        ...
        with tempfile.TemporaryDirectory(prefix="codex-storage-") as directory:
            ...
            try:
                ...
                # Fail before any CREATE when ledger 004 is missing. 005 is never
                # inserted in this fixture: independence is exercised by real SQL.
                ...
                # Existing trace DDL is reused unchanged, without migration 005.
